...
from application.use_cases.decide_approval import DecideApprovalCommandV1, DecideApprovalError, decide_approval
...
#: Only the decision route can return 503: `promotion_not_available` is raised by
#: `decide_approval` alone. Publishing it on the shared map told every generated
#: client that the two pure GETs and the create POST can return it too.
_DECISION_RESPONSES = {**_RESPONSES, 503: {"model": ProblemDetailsV1}}
...
_ERROR_STATUS = {
    ...
    "promotion_not_available": 503,
    "agent_run_not_cancellable": 409,
}

#: Detail per decision-route problem code. One shared sentence was wrong for two
#: of these: `promotion_not_available` means the approval IS valid and the server
#: capability is absent (Decision 3 fixes its literal wording and warns against
#: confusing 503 with the 409 "your binding is no longer valid" family), and
#: `approval_not_granted` is a policy refusal, not a stale binding.
_DECISION_DETAIL = {
    "promotion_not_available": "Baseline promotion is not available yet.",
...
def decide_approval_route(approval_id: UUID, body: ApprovalDecisionIn, idempotency_key: IdempotencyKey, connection: Connection = Depends(get_site_context), session: ResolvedSession = Depends(get_session), settings: Settings = Depends(get_settings), approvals: ApprovalRepository = Depends(get_approval_repository), audit_writer: AuditWriter = Depends(get_audit_writer), conversations: ConversationRepository = Depends(get_conversation_repository), schedule_runs: ScheduleRunRepository = Depends(get_schedule_run_repository), baselines: SiteBaselineReader = Depends(get_site_baseline_reader), now: datetime = Depends(get_clock)):
    ...
    if stored is not None:
        ...
        return _out(replayed, now)
    try:
        result = decide_approval(connection, command=DecideApprovalCommandV1(site_id=session.site_id, actor_id=session.app_user_id, approval_id=approval_id, decision=body.decision, expected_resource_version=body.expected_resource_version, request_id=uuid4()), approvals=approvals, schedule_runs=schedule_runs, baselines=baselines, audit_writer=audit_writer, conversations=conversations, scheduling_baseline_enabled=settings.scheduling_baseline_enabled, clock=lambda: now)
    ...
    except AgentRunNotQueuedError as exc:
        # TX3's agent half found the run outside `approval_required`. Nothing is
        # committed -- the exception propagates out of `decide_approval` and
        # `get_site_context` rolls the whole bundle back, so the binding is still
        # `pending`. Without this arm the global handler turned a recoverable
        # conflict into an untyped 500 (Decision 9's table has no 500 row).
        return problem_response(status=409, code="agent_run_not_cancellable", title="Approval decision could not be completed", detail=_DECISION_DETAIL["agent_run_not_cancellable"], extra=_context({"agent_run_status": "approval_required"}, {"agent_run_status": "changed"}))
    output = _out(result.binding, now)
