...
_RESPONSES = {403: {"model": ProblemDetailsV1}, 404: {"model": ProblemDetailsV1}, 409: {"model": ProblemDetailsV1}, 422: {"model": ProblemDetailsV1}, 503: {"model": ProblemDetailsV1}}
...
def _out(binding, now: datetime) -> ApprovalOut:
    ...
    return ApprovalOut(approval_id=binding.approval_id, state=state, schedule_run_id=binding.schedule_run_id, candidate_schedule_version_id=binding.candidate_schedule_version_id, baseline_schedule_version=binding.baseline_schedule_version, scenario_version_id=binding.scenario_version_id, consequence_summary=binding.consequence_summary, policy_version=binding.policy_version, parameter_hash=binding.parameter_hash, consequence_hash=binding.consequence_hash, agent_run_id=binding.agent_run_id, created_at=binding.created_at, expires_at=binding.expires_at, resource_version=binding.resource_version)
...
@router.post("/{approval_id}/decision", response_model=ApprovalOut, responses=_RESPONSES)
def decide_approval_route(approval_id: UUID, body: ApprovalDecisionIn, idempotency_key: IdempotencyKey, connection: Connection = Depends(get_site_context), session: ResolvedSession = Depends(get_session), settings: Settings = Depends(get_settings), approvals: ApprovalRepository = Depends(get_approval_repository), audit_writer: AuditWriter = Depends(get_audit_writer), conversations: ConversationRepository = Depends(get_conversation_repository), schedule_runs: ScheduleRunRepository = Depends(get_schedule_run_repository), baselines: SiteBaselineReader = Depends(get_site_baseline_reader), now: datetime = Depends(get_clock)):
    ...
    if stored is not None:
        ...
        if problem is not None:
            return problem_response(status=409, code=f"approval_{problem['outcome']}", title="Approval is no longer current", detail="Refresh or rerun before making another decision.", extra={"expected": problem["expected"], "current": problem["current"]})
    ...
    try:
        ...
        return problem_response(status=_ERROR_STATUS.get(exc.code, 422), code=exc.code, title="Approval decision could not be completed", detail="The approval is no longer valid for this decision.", extra={"expected": exc.expected, "current": exc.current})
    output = _out(result.binding, now)
    if result.outcome in ("expired", "stale"):
        ...
        return problem_response(status=409, code=f"approval_{result.outcome}", title="Approval is no longer current", detail="Refresh or rerun before making another decision.", extra={"expected": result.expected, "current": result.current})
    schedule_runs._store_idempotent_result(connection, site_id=session.site_id, actor_id=session.app_user_id, operation=operation, idempotency_key=idempotency_key, body_hash=body_hash, response_payload=output.model_dump(mode="json"))
    return output
