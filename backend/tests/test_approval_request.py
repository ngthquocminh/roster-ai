...
from application.use_cases.request_approval import RequestApprovalCommandV1, StaleBaselineVersionError, request_approval
...
def test_tx1_binds_only_the_exact_feasible_candidate_and_writes_success_audit() -> None:
    ...
    binding = request_approval(None, command=_command(runs), schedule_runs=runs, baselines=Baselines(), approvals=approvals, audit_writer=audit, conversations=conversations, approval_expiry_seconds=3600, scheduling_baseline_enabled=True, clock=lambda: NOW)
