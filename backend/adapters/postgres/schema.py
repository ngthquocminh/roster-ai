...
approval_request = Table(
    ...
    Column("conversation_id", UUID(as_uuid=True), nullable=False), Column("agent_run_id", UUID(as_uuid=True), nullable=True), Column("schedule_run_id", UUID(as_uuid=True), nullable=False), Column("candidate_schedule_version_id", UUID(as_uuid=True), nullable=False), Column("scenario_version_id", UUID(as_uuid=True), nullable=False), Column("baseline_schedule_version", String(100), nullable=True), Column("baseline_resource_version", BigInteger, nullable=True),
...
Index("uq_approval_request_pending_run", approval_request.c.site_id, approval_request.c.schedule_run_id, unique=True, postgresql_where=text("state = 'pending'"))
