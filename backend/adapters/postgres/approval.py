...
def _binding(row) -> ApprovalBindingV1:
    return ApprovalBindingV1(
        ...
        candidate_schedule_version_id=row.candidate_schedule_version_id, baseline_schedule_version=row.baseline_schedule_version,
...
class PostgresApprovalRepository:
    def create_pending(self, connection: Connection, *, binding: ApprovalBindingV1, pending_payload: dict | None) -> None:
        connection.execute(insert(approval_request).values(
            ...
            candidate_schedule_version_id=binding.candidate_schedule_version_id, baseline_schedule_version=binding.baseline_schedule_version,
    ...
    def get(self, connection: Connection, *, approval_id):
        row = connection.execute(select(approval_request).where(approval_request.c.id == approval_id)).one_or_none()
        return _binding(row) if row else None

    def list_for_schedule_run(self, connection: Connection, *, schedule_run_id):
        return tuple(_binding(row) for row in connection.execute(select(approval_request).where(approval_request.c.schedule_run_id == schedule_run_id).order_by(approval_request.c.created_at)).all())

    def get_pending_for_agent_run(self, connection: Connection, *, agent_run_id):
        row = connection.execute(select(approval_request).where(approval_request.c.agent_run_id == agent_run_id, approval_request.c.state == "pending")).one_or_none()
        return _binding(row) if row else None
