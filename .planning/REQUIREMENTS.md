...
- [x] **RES-01**: User can see coverage summary cards for a completed run
- [x] **RES-02**: User can see a demand-vs-served chart for a completed run
- [x] **RES-03**: User can see the schedule as a readable table
- [x] **RES-04**: User can fetch a plain-language insight report on demand; the UI branches on the response's `ready` field, not on the status code
- [x] **RES-05**: An insight failure (`502`) leaves the rest of the results view intact — a completed schedule is never invalidated by a failed report
- [x] **RES-06**: Degenerate-solve warnings recorded on the run (`SolveResult.warnings`) are surfaced rather than silently dropped
