...
**Status.** The engine, backend, and LLM layer described in this document
shipped in v0.3; the frontend (§5) shipped in v0.4. For current
phase/milestone status, see `.planning/`. The rest of the system is described
here for context and to keep the architecture honest.
...
rosterai/                           # one git repo
  ...
  docs/                             # ARCHITECTURE.md, API.md, README.md, vision.md, archive/
  README.md
```
...
## 5. Frontend (shipped) and deploy (not yet)

- **Frontend (shipped, v0.4).** Four-route shell under `frontend/` exactly per
  UI-SPEC: `/` (Home — scenario list/create from a backend-offered fixture),
  `/scenarios/:scenarioId` (Editor, mounted at `ScenarioLayout`'s index route —
  fixture-backed scenario, NL constraint box, applied-overrides list),
  `/scenarios/:scenarioId/runs` (RunHistory — trigger a run, poll status, list
  prior runs), `/scenarios/:scenarioId/runs/:runId` (ResultsView — coverage
  cards, demand-vs-served chart, on-demand insight report, scrollable schedule
  table). Vite + React 19 + TypeScript, TanStack Query for server cache,
  React Router for route state, shadcn/ui (Tailwind v4) + recharts, typed
  against `docs/API.md` via openapi-typescript/openapi-fetch codegen (zero
  hand-authored payload shapes). CORS middleware on the FastAPI app was the
  one backend change required to enable it (BE-01).

  **Deferred, not built:** a `WhatIfView` / what-if compare + delta
  explanation (deliberately held out of both v0.3 and v0.4 as a second large
  feature on top of a from-scratch React app — see `.planning/PROJECT.md`
  "Active" requirements), an input-upload endpoint (scenarios are created only
  from committed fixtures), and run cancellation + concurrency limits (single
  worker pool, no cancel path — v0.4 ships an honest uncancellable wait
  instead).
- **Deploy (not yet built).** Target is **AWS** — frontend → S3 + CloudFront;
  backend container → ECR + App Runner/ECS/EC2 (container compute, not
  Lambda — CP-SAT solves are CPU-heavy and long-running), matching §3.1 and
  `README.md`. No local-dev container orchestration exists yet (no
  `docker-compose.yml` in the repo). Out of scope until the feature set for a
  public-facing release is complete.
