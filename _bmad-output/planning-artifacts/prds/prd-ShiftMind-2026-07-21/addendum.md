...
Logfire export must use PydanticAI instrumentation with content and binary capture disabled by default, and an export-boundary sanitizer that forwards only allow-listed attributes for every span type (agent, HTTP, database, worker), strips URL query strings, never exports SQL parameter values, and records exceptions by type only. Never export credentials, raw workforce data, full prompts/completions, schedule payloads, tool arguments/results, or approval evidence, except through the two authorized live-evaluation channels, both carrying only conversations run on seeded synthetic fixtures and both tagged `deployment.environment=live-eval`: (1) the diagnostic mode `AGENT_TRACE_CONTENT_MODE=synthetic-eval`, set only by the disposable live-evaluation stack; (2) the live-evaluation publisher (Story 5.10), which replays a finished live-suite run report into a Logfire experiment and may export each turn's user message, authored obligation, visible reply and judge reasons from that report, nothing else from it. Credentials and exception text are withheld in every mode and channel. Client-supplied trace context is never trusted. (Channel 2 added 2026-09-25; see the amendment in sprint-change-proposal-2026-09-24.)
