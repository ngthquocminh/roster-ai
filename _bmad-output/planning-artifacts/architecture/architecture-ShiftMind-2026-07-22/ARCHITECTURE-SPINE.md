...
- **2026-09-17 clarification (Story 5.7):** the prose guard narrowed from "no numeric characters in agent prose" to "no numeral-bearing word absent from trusted text for the turn" (planner messages, persisted gate-passed conversation text, the workflow snapshot, this turn's tool results; UUIDs/long hex removed first). Copying an entity name such as `Forklift C01` or a planner-given value is not a numerical claim; a counted, summed, or otherwise derived quantity still requires a calculator-backed claim. The strict rule made real task names and planner values unstatable. Residual risk, owned by fact/effect checks rather than this guard: a wrong number coincidentally matching trusted text.
- **2026-09-17 clarification (Story 5.7):** plain text is accepted as a final answer, but only as a prose-only grounded answer (`TextOutput` on `GroundedAnswerV1` turns, `backend/agent/runtime.py`); the structured answer, clarification, refusal and draft outputs remain tools. With only output tools the request carries `tool_choice="required"`, and `openai/gpt-5.6-luna`, measured 2026-09-17, still replied in text after a tool result and, unable to end that text, repeated it until the upstream stream broke (`invalid_output`). Accepting text lets the provider send `tool_choice="auto"` so the reply terminates. Nothing is trusted more: the numeral-word guard above and the grounding gate apply to a prose-only answer exactly as to a tool answer, so a number in it still needs a cited, calculator-backed claim. A plain-text reply becomes a single prose segment and carries no claim of its own.
...
### AD-16 — Live conversation acceptance and deterministic invariant evidence [ADOPTED]
...
- **2026-09-15 correction:** AI conversational readiness requires Story 5.7's real-provider persisted conversation suite against the actual API/database/worker/solver. Missing or failing required live evidence blocks story completion and Gate B regardless of deterministic results. Live evidence is the acceptance authority for observed conversational behavior; deterministic evidence remains the authority for its tested application invariants. Existing `authoritative: false` on live evaluation records describes the latter distinction, never permission to ignore a live failure. The Gate B aggregator must require a separate `live_conversation_journeys` verdict, reject incomplete/stale evidence, and bind the current installed-tool/operation inventory. No release exception marks this required verdict passed. Authored test-only planner-visible transcripts may be retained as sanitized dedicated evidence; raw provider payloads, hidden reasoning, credentials, unrelated conversations, and production telemetry content remain excluded.
- **2026-09-17 right-sizing:** the Story 5.7 suite is three scenarios with one clean run each plus measured per-turn pass rates; live browser journeys are removed (sprint-change-proposal-2026-09-17). All other rules in the correction stand.
...
| Logfire SDK + pydantic-evals | `logfire==5.1.0`, `pydantic-evals==2.27.0` (dev group; Story 5.10) | dev/eval tooling only; never imported by api, worker, agent, application or domain |
