...
### Story 5.7: Prove Live Conversations Through Baseline Promotion [Corrective Insert]

As a planner, I want natural conversations to remain useful through investigation, proposal revision, real optimization, and explicit baseline approval, so that the demonstrated AI workflow works beyond isolated tool-routing prompts.

**Acceptance:** Implement the complete contract in [Story 5.7](../implementation-artifacts/5-7-prove-live-conversations-through-baseline-promotion.md) and its [eight-scenario catalogue](../implementation-artifacts/live-conversation-scenarios-5-7.md). Reproduce Minh's exact introduction/capability-question/typo sequence first; run at least eight varied-length scenarios (6–20 user turns, including a complete 20-turn conversation) with five scenario-specific independent live prefixes each; verify every installed tool and supported operation, useful grounded answers, actual persisted effects, and the draft → real solver → candidate → approval → baseline journey. Require three consecutive complete live runs plus browser reproduction/journey evidence on the configured provider and actual stack. Missing, skipped, failed, or partial required evidence blocks completion and Gate B. Deterministic passes cannot substitute, and release exceptions cannot mark this live obligation passed.

**Sequence:** after 5.6, before the next Gate B assessment. Existing 5.5/5.6 completion records retain their original limited scope. Story 5.7 owns product fixes discovered by these live conversations and a required `live_conversation_journeys` gate verdict; the matrix and gate are planned work until executable evidence exists.
...
| Required live conversation journeys | B | Story 5.7: at least eight distinct varied-length scenarios (6–20 user turns, including a complete 20-turn conversation); five scenario-specific independent prefixes each; every installed tool and supported operation; real persisted draft/solver/candidate/approval/baseline effects; three consecutive complete live matrix runs plus real browser reproduction and journey evidence on the same configured release stack. All required results pass. Missing, skipped, failed, partial, or stale evidence blocks. Deterministic results and the preceding row's exception mechanism cannot satisfy or waive this verdict. | Story 5.7; Gate B requires `live_conversation_journeys` |
