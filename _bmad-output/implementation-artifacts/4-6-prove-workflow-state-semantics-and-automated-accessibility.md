...
Still 60 states (27 primitive + 33 feature), so `STATE_MATRIX.length` is unchanged, but the
feature half now enumerates what the product actually renders rather than what the story author
typed. Two rule defects surfaced immediately on real components and were corrected — both were
...
**Coverage deliberately not carried over, and why.** These were prose entries with no product
behind them, so nothing real is lost, but Task 3 named them and the change must be visible:
`draft/queued-for-optimization` (needs `useStartScheduleRun` in its success state, which is
mutation state and unreachable without `vi.mock`); `comparison/not computed` (a null comparison is
handled by `ScenarioResults`, not by `ComparisonSummary`); and the five prose `terminal-outcome`
entries that duplicated run statuses already covered by `RunStatusBadge`, plus the three
`terminal-outcome/approval *` entries now covered by the real `approval` family. The
`message/approval rejected|expired|stale` prose entries are replaced by the seven real terminal
reasons the component actually has.
