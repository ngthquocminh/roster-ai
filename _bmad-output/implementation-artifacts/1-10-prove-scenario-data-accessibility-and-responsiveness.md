...
- [x] **Task 1: Stand up the two-layer accessibility harness** (AC: #1, #2)
  ...
  - [x] `color-contrast` cannot run in jsdom (no computed layout/paint). Run it **only** in Layer B. Do not assert contrast in Vitest.

- [x] **Task 2: Semantic, ARIA, and keyboard conformance (Layer A)** (AC: #1)
  ...
  - [x] Also sweep every entry in `frontend/src/components/primitives/fixtures.tsx` (Story 1.6's VR registry — `StatusBadge`, `InlineAlert`, `Skeleton`, `EmptyState`, `ReconnectBanner`, `EvidenceLink`, `EvidenceHighlight`, `IdentifierCopyButton`, all states). Iterate the exported registry; do not hand-list states, or the sweep silently rots when 1.6's registry grows.
  - [x] **Explicit assertions axe cannot make** (axe finds violations, not contract conformance — a clean axe run is necessary, not sufficient):
...
- [x] **Task 3: Implement and prove reduced motion** (AC: #1)
  - [x] **Implementation.** Add a global guard to `frontend/src/index.css` inside `@layer base`, honoring `@media (prefers-reduced-motion: reduce)`: neutralize `animation-duration`/`transition-duration` (`0.01ms !important`), `animation-iteration-count: 1`, and `scroll-behavior: auto`. This is the minimum that covers the unguarded Radix `data-open:animate-in`/`data-closed:animate-out` on `dialog.tsx:42,64`, `select.tsx:70`, `tooltip.tsx:45` and the `transition-*` on `button.tsx:8`, `badge.tsx:7`, `input.tsx:11`, `select.tsx:45`, `table.tsx:60`, `tabs.tsx:64,67`, `textarea.tsx:10`.
  - [x] **Do not** remove the animation classes themselves and do not fork the shadcn `ui/` components — `ui/` is vendored shadcn and the project's convention (Story 1.6) is ShiftMind deltas via tokens/global CSS, not component forks. `Skeleton`'s existing `motion-reduce:animate-none` stays; the global rule is additive.
  - [x] **Status text must survive.** EXPERIENCE.md:193 — reduced motion disables non-essential transition/spin, but every status string stays rendered. Assert in Layer B (with `emulateMedia({ reducedMotion: 'reduce' })`) that the pagination row-position status, the group loading `role="status"` label ("Loading scenario data", `ScenarioDataGroupState.tsx:30`), and the stale banner text are all still present and non-empty.
  - [x] **Evidence highlight never animates** (DESIGN.md:115 — "reduced-motion and default behavior are visually identical"; EXPERIENCE.md:144 — "evidence targeting never flashes or pulses"). Assert `EvidenceHighlight` renders identically under `reducedMotion: 'reduce'` and `'no-preference'`.

- [x] **Task 4: Implement and prove responsive behavior** (AC: #2)
  - [x] **Breakpoints are fixed by EXPERIENCE.md:179-181 — use these exact values, do not invent others:** desktop `≥1024px`, tablet `768–1023px`, phone `<768px`. Tailwind's `md` (768px) and `lg` (1024px) map exactly; use `md:`/`lg:` and never a custom breakpoint.
  - [x] **Desktop — already correct, verify only.** `ScenarioWorkspace.tsx:121` renders `<main className="px-6 py-8">` with no `max-w-*`, so Scenario Data already expands within the 24px gutter (DESIGN.md:94). Chat/Runs/Results keep the centered reading column. **Do not add a max-width to the workspace main.**
  - [x] **Tablet — controls stack.** `ScenarioDataView.tsx:52-56` currently emits a fixed `flex justify-end` chooser row + `FilterBar`'s `flex flex-wrap`. Make the control cluster stack below `lg`. Group navigation **already satisfies** the contract: EXPERIENCE.md:180 permits "a Select **or horizontal list**", and `ScenarioDataView.tsx:46-50` is a horizontally scrollable `TabsList` with `min-w-max`. **Do not replace the group Tabs with a Select** — that is unnecessary rework of a compliant surface. Same for `WorkspaceTabs.tsx:16` (`overflow-x-auto` + `min-w-max whitespace-nowrap`), which already keeps labels untruncated per EXPERIENCE.md:180.
  - [x] **Phone — compact essential-column default.** The only genuinely missing piece. `useColumnVisibility.ts` has no viewport awareness. Add a phone default that hides non-essential columns on first render below `768px`, subject to two hard invariants that already exist and must not be violated:
  ...
  - [x] **Phone read-only triage is otherwise already satisfied** — Scenario Data has no composer, draft, run, cancel, or approval control (Story 1.9's `scenarioDataBoundaries.test.ts` proves it). Do **not** build the "direct Maya to desktop" copy from EXPERIENCE.md:181; there is no consequential control at Gate A for it to gate. Note this scope call in completion notes.
  - [x] **Server authorization never depends on viewport** (AC #2's second clause — a *backend* obligation, easy to skip). New backend test (extend `backend/tests/test_scenario_projection.py` or the Story 1.9 file `backend/tests/test_gate_a_mutation_audit.py`): issue the same projection request with desktop, tablet, and phone `User-Agent` strings plus `Sec-CH-UA-Mobile: ?1`/`?0` and `Viewport-Width` headers, and assert byte-identical authorized responses **and** identical denial behavior for an unauthenticated/cross-site request. First-party backend code reads no UA or client hints today (verified: the only `user-agent` hits under `backend/` are vendored `authlib`) — this test is the regression lock on that fact.

- [x] **Task 5: Browser-level proofs — zoom, reflow, text spacing, targets, contained scroll (Layer B)** (AC: #1, #2)
  - [x] Run the Task-1 axe configuration (all five WCAG tags + `target-size` enabled + `color-contrast`) at each viewport — desktop `1280×800`, tablet `900×1024`, phone `390×844` — over `/` and `/scenarios/:id/data` for **every** group, with a filter applied, a column hidden, and the chooser open. Zero violations.
  - [x] **200% zoom.** Playwright has no zoom API; the accepted equivalent is halving the CSS viewport at the same content scale (`1280×800` → `640×400`). Assert at that size: no page-level horizontal scroll, no control hidden or clipped, all status text still readable.
  ...
  - [x] **Text spacing (WCAG 1.4.12).** `page.addStyleTag` with the standard bookmarklet values — `line-height: 1.5`, `letter-spacing: 0.12em`, `word-spacing: 0.16em`, paragraph spacing `2em` — applied to `*`. Assert no clipped or overlapped content and no lost control. Pay attention to `whitespace-nowrap` on `WorkspaceTabs.tsx:17` and `badge.tsx`/`button.tsx`, and to the `line-clamp-1` on `select.tsx:45`'s value.
  - [x] **Touch targets ≥ 44×44 CSS px** (EXPERIENCE.md:183 — stricter than axe's WCAG 2.5.8 threshold of 24px, so axe passing is not enough). Measure `boundingBox()` for every interactive element in Scenario Data. Known-good: `IdentifierCopyButton` (`min-h-11 min-w-11`, `IdentifierCopyButton.tsx:48`), pagination/Apply/Clear/chips/chooser trigger (`min-h-11`). **Known risks to fix, not to explain away:** the sort header `<button className="min-h-11">` (`ScenarioDataTable.tsx:18`) has no min-width — a short header like "Unit" will be under 44px wide; and `DropdownMenuCheckboxItem` (`ui/dropdown-menu.tsx:20`, `py-2 text-sm`) is ~36px tall. Fix by adding width/height at the **call site** (`ScenarioDataTable.tsx`, `ColumnChooser.tsx`), not by editing vendored `ui/` primitives.
  - [x] **Sticky header opacity** (DESIGN.md:100 — "text never layers visually over scrolled content"). `ScenarioDataTable.tsx:33` sets `[&_thead]:sticky [&_thead]:bg-muted`. Assert the computed `background-color` of the sticky `thead` is fully opaque (alpha `1`) at every viewport.
  - [x] **Keyboard-only journey, real browser:** sign-in-stubbed load → catalogue row activation → workspace tab → group tab → sort → filter Apply → page Next → copy an identifier → hide a column — entirely via `keyboard.press`, asserting a visible focus indicator at every stop (`focus-visible:ring-3 focus-visible:ring-ring/50` is the established pattern) and that focus is never lost to `<body>`.

- [x] **Task 6: Manual screen-reader pass (NVDA) — recorded, not automated** (AC: #1)
  - [x] EXPERIENCE.md:196 names NVDA on Windows in the portfolio-minimum matrix, and AC #1 says "screen-reader use". No automated tool proves this; **do not claim it via axe.** Write `docs/ACCESSIBILITY-NVDA-CHECKLIST.md` (flat, uppercase — matches the existing `docs/TESTING.md`, `docs/API.md` convention; `docs/` has no topic subdirectories) with one row per AC-#1 obligation — heading announcement on route change, table caption + column-header association, `aria-sort` change announcement, row-position announcement on page change, "Copied {identifier type}", the evidence-reveal explanation, and the disabled-Results explanation (`WorkspaceTabs.tsx:33-45`) — each with expected utterance, observed utterance, and pass/fail.
  - [x] Execute it once against `npm run preview` with NVDA + Chrome and NVDA + Edge, and record the dated result. If NVDA is unavailable in the execution environment, record `"nvda_manual_pass": "not executed — <reason>, <date>"` in Task 7's evidence and flag it in completion notes. **Do not report it as passed, and do not silently omit the field** — same posture Story 1.9 took with `legacy_route_live_flag_state`.

- [x] **Task 7: Record Gate A evidence** (AC: #1, #2)
  - [x] Write `evidence/story-1.10/scenario-data-accessibility-and-responsiveness.json`, following the exact shape of `evidence/story-1.9/gate-a-viewer-parity-and-mutation-denial.json` (read it first; it is the template): `story`, `requirements` (`NFR18`, `NFR20`, `UX-DR26`–`UX-DR29`, `UX-DR31`), `measurement_date`, `fixtures` with the two Gate A fixture identities, per-check `results`, `test_evidence` commands + counts, `version_bindings` (NFR27: dataset/evaluator/tool/policy/application/scenario/code/image), and top-level `passed`.
  - [x] `results` must name each check separately — `axe_jsdom`, `axe_browser`, `keyboard_journey`, `reduced_motion`, `zoom_200`, `text_spacing`, `no_page_horizontal_scroll`, `touch_targets_44px`, `contained_table_scroll`, `responsive_desktop`, `responsive_tablet`, `responsive_phone`, `authorization_viewport_independent`, `nvda_manual_pass` — plus `browsers_tested` and `viewports_tested`. A single aggregate `"passed": true` with no per-check breakdown is not acceptable: NFR29 requires a failure to name the exact gate.
  - [x] Bind `axe_core_version`, `playwright_version`, and the enabled-rule delta (`target-size: enabled`) so a future axe upgrade that changes rule coverage is visible in the record.

- [x] **Task 8: Full regression gate**
  - [x] Frontend: `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, `npm run test:e2e`. Report file/test counts before and after (baseline at `f27bafd`: **47 files, 200 tests**).
  - [x] Backend: `uv run --frozen pytest` from `backend/` (baseline **349 passed, 6 deselected**) — Task 4's viewport-independence test is the only backend addition. `uv run --frozen pytest -m postgres` needs a live local PostgreSQL and skips cleanly without one. `alembic check` must show zero diff (this story adds no migration).
  - [x] **Re-run Story 1.9's Gate A guards explicitly** and report them by name: `ScenarioDataParity.test.tsx`, `scenarioDataBoundaries.test.ts`, `legacyReachability.test.ts`. The phone column default (Task 4) is the most likely thing to perturb the parity test's rendered-cell expectations — if it does, set the parity test's environment to the desktop viewport rather than loosening its assertions.
  - [x] Record the new `devDependencies` and their versions in completion notes; ARCHITECTURE-SPINE.md:263 requires planned stack rows to be "added to manifests and lockfiles by their implementation gate before use" — commit the `package-lock.json` change.

### Review Findings

- [x] [Review][Patch] Second Gate-A fixture (`sample_tiny_input_more_tm`) is claimed as exercised by this story's evidence but is never wired into the new a11y/e2e harness — **fixed**: `apiStubs.ts`'s `installApiStubs` now accepts a `{ fixture: "tiny" | "more_tm" }` option (contract JSON keyed by `CONTRACT_FILES`); added `frontend/e2e/gate-a-fixture-coverage.spec.ts` exercising the `workers` group (the only group where `more_tm` differs — 22 rows vs. 10, verified by diffing the two contract JSONs) with an axe-clean sweep plus a row-count and contained-scroll assertion. Verified passing on chromium + msedge. [frontend/e2e/support/apiStubs.ts, frontend/e2e/gate-a-fixture-coverage.spec.ts]
- [x] [Review][Patch] `useColumnVisibility`'s malformed/error sessionStorage paths return a truthy empty `Set` instead of `null`, silently defeating the phone essential-column default — **fixed**: both paths now return `null` so `??` falls through to the phone default; added a regression test ("falls back to the phone essential-column default when stored preference is hostile"). [frontend/src/features/scenario-data/useColumnVisibility.ts:15,17,20]
- [x] [Review][Patch] Reduced-motion "evidence highlighting identical" e2e test fabricates a bare `<div>` with a hand-copied className instead of rendering the real `EvidenceHighlight` component — **fixed**: exported `EVIDENCE_HIGHLIGHT_CLASS` from the component (also used internally now); the e2e/ TS project can't cross-import into src/ (`tsconfig.e2e.json` project-reference boundary), so the spec instead reads the constant out of the real component source at test time via a regex, and fails loudly if it can't find it — still breaks on drift instead of silently passing. [frontend/e2e/reduced-motion.spec.ts, frontend/src/components/primitives/EvidenceHighlight.tsx]
- [x] [Review][Patch] "Column hidden" state is never axe-scanned at the page level — only the open dropdown menu is re-scanned after hiding a column — **fixed**: added a full-page `expectAxeClean(page)` after closing the chooser, in addition to the existing menu-scoped scan. [frontend/e2e/accessibility.spec.ts:36-40]
- [x] [Review][Patch] Layer A (jsdom/Vitest) axe runs omit the mandated `.withTags([...5 WCAG tags])` / `target-size: enabled` configuration — **fixed**: `expectClean()` now passes `runOnly: { type: "tag", values: WCAG_TAGS }` and `target-size: { enabled: true }`, mirroring Layer B's config. [frontend/src/test/accessibility.test.tsx:52-53]
- [x] [Review][Patch] `apiStubs.ts`'s `pageFor()` ignores every filter query param, so the "with a filter applied" axe-clean state rendered DOM-identical to the unfiltered state — **fixed**: added `matchesFilters()` (exact/`_contains`/`_gte`/`_lte` matching mirroring the backend's filter semantics); `total_count`/`matching_count` now correctly diverge. `accessibility.spec.ts`'s filter values were updated to real IDs from the fixture (were UUID-shaped placeholders that never matched). This also surfaced and fixed a **pre-existing** dependency: `keyboard-journey.spec.ts` selected `family=indirect` (only 6 rows) expecting a "Next" page that no longer exists once filtering is real — switched to `family=outbound` (493 rows). [frontend/e2e/support/apiStubs.ts:44-58, frontend/e2e/accessibility.spec.ts, frontend/e2e/keyboard-journey.spec.ts]
- [x] [Review][Patch] `ScenarioDataTable.test.tsx`'s rewrite dropped the pre-existing assertion that the initial (no active sort) state renders `aria-sort="none"` — **fixed**: restored an initial render with no `sort` prop and the `aria-sort="none"` assertion, before progressing to ascending/descending, for every group. [frontend/src/features/scenario-data/ScenarioDataTable.test.tsx:14]
- [x] [Review][Patch] `usePhoneViewport.ts`'s `useEffect` calls `window.matchMedia` without the feature-detection guard used elsewhere in the same file, and no test exercises the live `matchMedia("change")` resize behavior — **fixed**: added the guard; created `usePhoneViewport.test.tsx` covering initial state, a live change-event flip without remount, and listener cleanup on unmount. [frontend/src/features/scenario-data/usePhoneViewport.ts:13]
- [x] [Review][Patch] Backend `test_projection_authorization_is_viewport_independent`'s "denied" branch is framed as proving cross-site rejection, but the added `Origin: https://cross-site.invalid` header is inert for GET requests (only unsafe methods are origin/CSRF-checked) — the 401 is proven only by the cleared session cookie — **fixed**: dropped the misleading `Origin` header, renamed the variable, and added a comment explaining the middleware only origin-checks unsafe methods. [backend/tests/test_scenario_projection.py:660-671]
- [x] [Review][Patch] `playwright.config.ts`'s `webServer.command` hardcodes a Windows `cmd /c` shell wrapper, which is not just redundant (Playwright already spawns `command` through a shell) but breaks the command on non-Windows hosts — **fixed**: dropped the wrapper down to the plain npm command. `msedge` stays in `projects` per Task 1's explicit EXPERIENCE.md-matrix requirement; verified the full suite still passes on both `chromium` and `msedge` after the change. [frontend/playwright.config.ts]
...
- 2026-08-06: Task 1 RED — e2e TypeScript failed on missing accessibility/API-stub modules; GREEN — typed harness, pinned tooling, deterministic API routes, Chromium axe smoke, typecheck, build, and 49-file regression passed.
- 2026-08-06: Task 2 RED — keyboard-order contract exposed the Radix tabpanel as an intentional focus stop and empty fixture groups as non-table states; GREEN — registry/state axe sweep plus explicit semantics and interaction contracts passed (279/279 full Vitest suite).
- 2026-08-06: Task 3 RED — Chromium reported the unguarded button transition at 0.15s; GREEN — global reduced-motion CSS, loading/row/stale status survival, and identical non-animated evidence highlight passed (3/3 Chromium; 279/279 Vitest regression).
- 2026-08-06: Task 4 RED — phone-default descriptor/hook tests failed against absent `essential` metadata and viewport support; GREEN — desktop/tablet/phone browser proofs, persistent override, evidence reveal, and viewport-independent authorization passed (283/283 frontend; 350 backend passed, 6 deselected).
- 2026-08-06: Task 5 RED — browser geometry found the Family sort target narrower than 44px and Edge exhausted the initial geometry/keyboard runtime budgets under full concurrency; GREEN — call-site target sizing plus unchanged-coverage timeout budgets passed all 44 Chromium/Edge tests.
- 2026-08-06: Task 6 — NVDA was absent from command lookup and standard system/user install locations; checklist and dated not-executed results recorded without treating axe as a manual pass.
- 2026-08-06: Tasks 7–8 — published version-bound evidence; full frontend, backend, PostgreSQL, Alembic, and named Story 1.9 guard gates passed.

### Implementation Plan

- Build the mandatory jsdom + Playwright harness first, then prove the existing semantic contract without rebuilding shipped primitives.
- Add only the two missing shipping behaviors (global reduced motion and breakpoint-aware viewing defaults), with regression locks at their owning boundaries.
- Finish with browser geometry/interaction proofs, manual-test evidence, and the full Gate A regression matrix.

### Completion Notes List

- Tasks 1–2 complete: added pinned axe/Playwright tooling, deterministic browser API fixtures, typed e2e wiring, 79 new Layer-A tests, and expanded existing ARIA/live-region/Escape/focus contracts. `npm test`: 49 files, 279 tests passed.
- Task 3 complete: global reduced-motion handling neutralizes animation and transitions without removing status meaning; all three real-browser proofs pass.
- Task 4 complete: fixed `lg` control stacking and `<768px` essential-column defaults preserve required/evidence fields and existing session storage. Gate A remains read-only on phone; no premature desktop-direction copy was added because no consequential control exists yet. The essential sets are authored in `columns.ts` for product review.
- Task 5 complete: Chromium and Edge prove zero configured axe violations, keyboard-only operation, reduced motion, 200% zoom-equivalent reflow, WCAG text spacing, 44×44 targets, contained narrow-viewport table scroll, opaque sticky headers, and no page-level horizontal overflow.
- Task 6 complete with the story-defined unavailable-environment posture: the NVDA checklist exists, but Chrome/Edge observations are explicitly `not executed` because NVDA is not installed. The evidence aggregate remains `passed: false`; a Windows NVDA workstation must complete the manual matrix before Gate A can be reported as fully passed.
- Tasks 7–8 complete: evidence binds fixture digests, browser/tool versions, rule delta, source baseline, and per-check outcomes. Frontend moved from 47 files / 200 tests to 49 files / 283 tests; Playwright passed 44/44; backend moved from 349 to 350 passed with 6 deselected; PostgreSQL markers passed 27/27; Alembic reported no new upgrade operations; `ScenarioDataParity.test.tsx`, `scenarioDataBoundaries.test.ts`, and `legacyReachability.test.ts` passed 19/19 explicitly.
- Added development dependencies and lockfile bindings: `axe-core@4.13.0`, `jest-axe@11.0.0`, `@playwright/test@1.62.1`, `@axe-core/playwright@4.12.1`, and `@testing-library/user-event@14.6.1`.
...
- frontend/e2e/support/accessibility.ts
...
- frontend/src/features/scenario-data/usePhoneViewport.ts
...
- 2026-08-06: Implemented and validated the two-layer accessibility harness, reduced-motion behavior, responsive viewing defaults, cross-browser geometry/keyboard proofs, viewport-independent authorization regression, manual NVDA checklist, and version-bound Gate A evidence. Moved to review with the manual NVDA gate explicitly outstanding.
