...
- [x] [Review][Patch] `calculate_candidate_metrics` crashes uncaught on any unserved volume-demand interval, which is the normal outcome of a real solve — `application/scheduling/candidate_metrics.py:51-57` raises a bare `ValueError` when a `volume`-family demand row has zero overlapping assignments, because its "required minutes" formula depends on the average rate of the *specific* assigned workers (a supply-side fact) rather than a fixed function of the demand row. `finalize_schedule_run.py:64-78` only catches `HardConstraintViolation`, so the exception propagates uncaught out of `execute_schedule_run`, leaving the run stuck in `solver_running` with no terminal status ever persisted. Task 6's own measurement reports ~247 unmet hours on a real solve of `sample_tiny_input.json`, so unserved volume intervals are the ordinary case, not an edge case. **Resolved (2026-08-20):** compute the volume-conversion rate from all workers qualified for the task (`ValidationFactsV1.workers`), not from the specific assignment set — this also fixes the accompanying non-determinism (required-minutes would otherwise differ across candidates staffing the same interval differently). Fall back to raising only when literally no one anywhere is qualified for the task, per DOMAIN-MODEL.md §3's fail-closed-not-zero principle; do not default to `required=0`, which would render an unstaffed interval indistinguishable from a fully-covered one.
- [x] [Review][Patch] `scale_demand` is the only one of the five soft-constraint kinds whose `ConstraintResultV1` is fabricated rather than measured — `application/scheduling/candidate_metrics.py:117-118` hardcodes `satisfied=True, measured=1.0` regardless of the actual assignments, contradicting Decision 6's claim that all five kinds get `satisfied` "measured post-solve from the assignments." No test constructs a `scale_demand` constraint and inspects the result. **Resolved (2026-08-20):** unlike the other four kinds (round-2 penalty terms verifiable against assignments), `scale_demand` rescales the task's demand amount directly inside round 1 (`engine/cpsat/builder.py:_aggregate_demand`, before any assignment exists) — so the only honest post-solve fact is whether the solve delivered against the scaled target. Thread per-task required/served minutes (already computed per-row in `calculate_candidate_metrics`'s main loop, just needs aggregating by `task_id`) into `_soft_constraint_result`: `measured` = served minutes for the task, `limit` = the task's original required minutes × `constraint.factor` (the calculator only sees unscaled demand, so the factor must be reapplied here), `unit="minutes"`, `satisfied = measured >= limit`.
- [x] [Review][Defer] The scenario-staleness check enforcing AD-9's "no silent rebasing" locks the proposal row (`for_update=True`) but reads the scenario context with no lock — `application/use_cases/create_run_snapshot.py:70-95`. Under READ COMMITTED, a concurrent scenario-version change landing between the context read and the snapshot INSERT is not guarded against in the same transaction. Closing it means extending `ScenarioCatalogueReader` with a locking read, which touches `backend/adapters/postgres/scenario_catalogue.py` — a file this story explicitly mandates as a zero-line diff [backend/application/use_cases/create_run_snapshot.py:70-95] — deferred, out of scope per the zero-line-diff mandate
- [x] [Review][Patch] AD-7's mandated wall-time-vs-other-ceiling status split is dead code — `governed_adapter.py`'s `solve()` never sets `SolverOutcomeV1.reason`, so `finalize_schedule_run.py`'s `deterministic_budget_exhausted` branch can never fire outside a hand-built test fixture; every deterministic-ceiling-only exhaustion is misclassified as `solver_timed_out` instead of the story's own decided `solver_failed`/`budget_exhausted` [backend/engine/governed_adapter.py:196-240]
- [x] [Review][Patch] Round 1's budget check discards its return value — if `apply_remaining_budget()` returns `False` before round 1 starts, the code solves anyway with no configured wall-time bound, unlike the symmetric check made before round 2 [backend/engine/governed_adapter.py:210-213]
- [x] [Review][Patch] Corruption-fixture test doesn't verify Task 8's exclusivity requirement ("its own check fail and only its own") — asserts `expected in failed` instead of `failed == {expected}`, so a mutation tripping two guards at once would pass undetected [backend/tests/test_candidate_validation.py:76-81]
- [x] [Review][Patch] Two independent clocks record the same terminal run transition — the injectable `clock` passed to `finalize_schedule_run` is only used for `candidate.created_at`; the repository independently calls `datetime.now(timezone.utc)` for `finished_at` on every non-candidate terminal run, bypassing the injected clock [backend/adapters/postgres/schedule_run.py:135]
- [x] [Review][Patch] The Decision-4 reproducibility test only asserts on the 3 deterministic solves it runs; the 3 multi-worker solves in the same test are printed but never checked, so the "trade is measured, not guessed" claim isn't actually re-checkable by CI [backend/tests/test_governed_solver_adapter.py]
- [x] [Review][Defer] AD-20-required contract fields are structurally dead — `RunSnapshotV1.objectives` is hardcoded to `()` (`ProposalV1` has no "objectives" distinct from constraints to source it from), and `ConstraintResultV1.contributing_evidence_refs`/`SolverOutcomeV1.evidence_refs` are never populated by any path in this story. They satisfy contract presence-validation but never carry real data via any reachable path today [backend/application/use_cases/create_run_snapshot.py:115] — deferred, pre-existing gap in the AD-20 field set that needs a future story to wire real objectives/evidence-ref sourcing once available
