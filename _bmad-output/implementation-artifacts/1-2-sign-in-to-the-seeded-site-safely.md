...
- [x] [Review][Decision][Resolved] Identity/session store had no correct privilege-bounded code path — `PostgresIdentitySessionStore` and `seed_planner.py` never assumed `shiftmind_runtime`; every login/callback/session/logout operation ran under the raw `database_url` connection, which per `docker-compose.yml` (`POSTGRES_USER=rosterai`) is a Postgres superuser that bypasses RLS and every REVOKE the migration puts on `auth.session_index`/`auth.login_handshake`. **Resolution (tightened hybrid of options 1+4, per dev sign-off):** introduced `shiftmind_login` — a real `LOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOBYPASSRLS` role, member of `shiftmind_runtime`, owning no grants of its own — as the API's actual connection identity. Added four new owner-held `SECURITY DEFINER` functions alongside the existing `auth.resolve_session` (`auth.create_login_handshake`, `auth.consume_login_handshake`, `auth.establish_session_for_subject`, `auth.revoke_session`; fixed `search_path`, `REVOKE ALL FROM PUBLIC`, `GRANT EXECUTE TO shiftmind_runtime` only), each schema-qualified with no dynamic SQL. `establish_session_for_subject` derives `app_user_id`/`site_id` from `idp_subject` *inside* Postgres in one atomic INSERT…SELECT — the caller can never supply an arbitrary identity/site pair. `PostgresIdentitySessionStore` now calls only these functions (never touches `auth.*`/`membership` tables directly) and `SET LOCAL ROLE shiftmind_runtime` on every connection, mirroring `get_site_context`'s existing pattern. `Settings` gained `provisioning_database_url` (defaults to the privileged `rosterai` connection) used only by Alembic (`migrations/env.py`) and the operator-only `seed_planner.py`; `database_url` (used by the API's `get_identity_store`/`get_site_context`) now defaults to `shiftmind_login`. `backend/tests/test_identity_role_boundaries.py` proves behaviorally (not just via static grant checks) that `shiftmind_login` cannot bypass RLS, cannot read/write `auth.*` tables directly, cannot `SET ROLE` to the provisioning role, and that the full handshake→session→resolve→revoke lifecycle succeeds end-to-end under the restricted role, including single-use handshakes and revoked-membership rejection. `ARCHITECTURE-SPINE.md`'s AD-23 wording updated from "API may only execute `auth.resolve_session`" to an explicit minimal function whitelist. `backend/scripts/gate_a_cutover.py` (Story 1.1) got the same `provisioning_database_url` fix as `seed_planner.py` — a follow-up commit after the same review conversation caught that leaving it on `database_url` would have been a live regression, not pre-existing debt. **Follow-up commit closed the one remaining gap too:** added `shiftmind_owner` — `NOLOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE BYPASSRLS` (BYPASSRLS is deliberate, not a relaxation — its SECURITY DEFINER functions must read the FORCE-RLS `membership` table before any `app.site_id` exists, and its safety comes from being unreachable: NOLOGIN, no runtime credential ever holds membership in it, only the migrator's ambient superuser status can `SET ROLE` into it). All five `auth.*` functions, both control tables, and the `auth` schema itself are now owned by it instead of by `rosterai`; `shiftmind_owner` also needed its own `SELECT` grants on `public.app_user`/`public.membership` since it doesn't own those (they're domain tables). Tests now assert `pg_tables.tableowner`/`pg_proc.proowner` directly and prove `shiftmind_login`/`shiftmind_runtime` cannot `SET ROLE shiftmind_owner`. `ARCHITECTURE-SPINE.md`'s AD-23 text needed no edit here — it already specified this shape correctly; the gap was pure implementation lag (predating this story — Story 1.1 had no `SECURITY DEFINER` functions, so the missing `shiftmind_owner` was inert until Story 1.2 introduced the first one). [`backend/adapters/postgres/identity.py`, `backend/migrations/versions/5e2a4c9d1f70_add_seeded_site_identity.py`, `backend/settings.py`, `backend/scripts/seed_planner.py`, `backend/scripts/gate_a_cutover.py`, `backend/migrations/env.py`, `backend/conftest.py`, `backend/api/routers/auth.py:120-146`, `backend/application/ports/session.py`, `backend/tests/test_identity_role_boundaries.py`, `backend/tests/test_postgres_integration.py`]
- [x] [Review][Patch] Sign-out never terminates the upstream IdP session — `logout()` only revokes the local `session_index` row; `OidcProvider.end_session_url()` is implemented on both adapters but never called, so a "signed out" user is silently re-authenticated by a still-live Cognito/IdP SSO session. [backend/api/routers/auth.py:201-222]
- [x] [Review][Patch] Several DB/provider calls in the auth flow have no exception handling and will surface a raw Starlette 500 instead of the RFC7807 `application/problem+json` shape the story establishes for `/api/v1/*` — `login()`'s `create_login_handshake`/`authorization_url`, `callback()`'s `resolve_active_identity`/`create_session`, and `enforce_versioned_session_and_csrf`'s `store.resolve_session` call. [backend/api/routers/auth.py:72-78, backend/api/routers/auth.py:120-146, backend/api/main.py:157-160]
- [x] [Review][Patch] `session_ttl_s = int(os.environ.get("SESSION_TTL_S", "3600"))` has no error handling — a misconfigured env var crashes `default_settings()`, which runs on every request via `Depends(get_settings)`, taking the whole API down. [backend/settings.py:114]
- [x] [Review][Patch] `_PUBLIC_VERSIONED_PATHS` is an exact-match set with no trailing-slash normalization — a request to e.g. `/api/v1/auth/login/` is treated as protected (401) instead of reaching the public login redirect, since the auth middleware runs before Starlette's own slash-redirect logic. [backend/api/main.py:123-148]
- [x] [Review][Patch] CSRF token is deterministically derivable from the session token alone (`hmac.digest(key=session_token, msg=b"shiftmind-csrf-v1", ...)`) rather than an independently generated secret with its own random value — not currently exploitable (the session token is itself HttpOnly-protected), but any future leak of the session token also defeats CSRF for free, with no per-deployment pepper. [backend/api/auth_security.py:16-22]
- [x] [Review][Patch] Deep-link redirect target is captured by the frontend but never consumed — `RequireSession.tsx` stashes `location.state.from` on redirect to `/signin`, but `SignIn.tsx` never reads it, and the backend `login`/`callback` flow always redirects to the fixed `settings.app_base_url`. A user bounced from a deep link always lands on the app root after signing in. [frontend/src/routes/RequireSession.tsx:14-19, frontend/src/routes/SignIn.tsx, backend/api/routers/auth.py:69]
- [x] [Review][Patch] Frontend sign-out has no query-cache invalidation and no error handling — `useSignOut()` never invalidates/clears the cached `["auth","session"]` query on success, and `AppBar.tsx`'s `handleSignOut()` has no try/catch around `signOut.mutateAsync()`, so a failed sign-out (CSRF/network error) leaves the user on the page with no feedback and no navigation. [frontend/src/hooks/useSession.ts:17-21, frontend/src/components/layout/AppBar.tsx:21-24]
- [x] [Review][Patch] `RequireSession.tsx` treats any `useSession()` query error — including a transient network/5xx failure, not just a genuine 401 — as "not authenticated" and redirects to `/signin`, bouncing an already-authenticated user off a protected route during a backend hiccup. [frontend/src/routes/RequireSession.tsx:13-20]
- [x] [Review][Patch] `CognitoOidcProvider` has no HTTP timeout on its discovery/JWKS/token-exchange calls, and caches discovery metadata + JWKS indefinitely for the process lifetime with no TTL or `kid`-miss refetch — a hanging Cognito endpoint blocks login indefinitely, and a routine signing-key rotation breaks every sign-in with generic 401s until the process restarts. This adapter has zero test coverage in the diff (only the fake provider is exercised). [backend/adapters/cognito/oidc.py]
- [x] [Review][Patch] `get_site_context`'s cleanup `finally` block re-executes SQL (`set_config('app.site_id', '', true)`) without its own guard — if the transaction was already aborted by an exception during `yield`, this cleanup statement itself raises `InFailedSqlTransaction`, masking the original error in the traceback the client/logs actually see (rollback itself still happens correctly via the surrounding `with engine.begin()`). [backend/api/deps.py:118-124]
- [x] [Review][Defer] No cleanup/retention for expired `auth.login_handshake` / `auth.session_index` rows [backend/migrations/versions/5e2a4c9d1f70_add_seeded_site_identity.py] — deferred, pre-existing (operational housekeeping reasonably arrives with Epic 3's job-queue infrastructure; no AC requires it now)
- [x] [Review][Defer] No `X-Frame-Options` / CSP `frame-ancestors` header on the new sign-in surface [backend/api/main.py] — deferred, pre-existing (clickjacking hardening is an app-wide/infra concern better addressed holistically, not called for by any of this story's ACs)
...
- 2026-07-26 — Code review: resolved the decision-needed finding on identity/session privilege boundaries. Added the `shiftmind_login` restricted API role and four `auth.*` SECURITY DEFINER functions (`create_login_handshake`, `consume_login_handshake`, `establish_session_for_subject`, `revoke_session`); `PostgresIdentitySessionStore` now runs exclusively through them under `SET LOCAL ROLE shiftmind_runtime`, never touching `auth.*`/`membership` tables directly. Split `Settings.database_url` (restricted, API) from new `Settings.provisioning_database_url` (privileged, Alembic + `seed_planner.py`). Added `backend/tests/test_identity_role_boundaries.py` (5 new tests) proving the boundary behaviorally; updated `ARCHITECTURE-SPINE.md` AD-23 wording. Full suite: 245 backend tests pass (was 238), including 18 `-m postgres` tests and the migration upgrade/downgrade/upgrade round trip.
- 2026-07-26 — Code review: applied all 10 `patch` findings. Sign-out now surfaces the IdP's `end_session_url` via an `X-Post-Logout-Redirect` header (fake/default config unaffected — always `None`); `/auth/login` accepts a validated same-origin `return_to` and the frontend now sends the original deep-link path through sign-in; CSRF derivation now takes a per-deployment `Settings.csrf_secret` instead of a hardcoded literal; `SESSION_TTL_S` misconfiguration falls back to the default instead of crashing every request; the public-auth-path check normalizes trailing slashes; `login()`/`callback()`/the versioned-API middleware now map DB/provider failures to RFC7807 502s instead of raw 500s; `CognitoOidcProvider` gained HTTP timeouts, hourly discovery/JWKS refresh, and a kid-miss retry; `get_site_context`'s cleanup no longer masks the original error on an aborted transaction; the frontend clears the cached session on sign-out and surfaces sign-out failures instead of failing silently; `RequireSession` now distinguishes a transient query error from a genuine unauthenticated state. All fixes covered by new tests. Also updated `backend/.env.example` for the `shiftmind_login`/`ROSTERAI_PROVISIONING_DATABASE_URL` split and new `CSRF_SECRET`, and fixed `backend/scripts/gate_a_cutover.py` to use `provisioning_database_url` (the same regression class as `seed_planner.py`, caught and fixed in a follow-up rather than left deferred — a test-verification pass for this one briefly wrote two rows into the real local dev database via its unpatched default; cleaned up directly against Postgres). Final suite: 258 backend tests pass (was 238 pre-review), 250 frontend tests pass (was 245); TypeScript, lint, and production build pass. Status moved to done.
- 2026-07-26 — Follow-up: closed the one remaining tracked gap from the privilege-boundary fix by adding `shiftmind_owner` (`NOLOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE BYPASSRLS`) and transferring ownership of all five `auth.*` SECURITY DEFINER functions, both control tables, and the `auth` schema to it, matching `ARCHITECTURE-SPINE.md` AD-23's already-correct wording for the first time. Removed the corresponding `deferred-work.md` entry. 2 new tests assert ownership directly (`pg_tables`/`pg_proc`) and that no runtime role can `SET ROLE` into it. Full suite: 260 backend tests pass.
