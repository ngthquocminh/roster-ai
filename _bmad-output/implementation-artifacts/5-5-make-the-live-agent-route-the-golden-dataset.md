...
- [x] [Review][Patch] Evidence regenerated on a dirty tree with no recorded justification
      ...
      green. **Still owed:** a clean-tree regeneration once this story's code is actually committed,
      to replace this `--allow-dirty` artifact with a reproducible one. **This is why story status
      stays `in-progress`, not `done`, after this review.**
