...
- [ ] **Task 1: `CapabilityManifestV1` — the canonical AD-20 contract** (AC: #1)
  - [ ] New file `backend/application/contracts/capability_manifest.py`. Frozen dataclass, module
  ...
  - [ ] Carry every field of `InspectCapabilityManifest` (`scheduling_inspect.py:123-139`) —
  ...
  - [ ] `approval_policy` becomes a closed `ApprovalPolicyV1 = Literal["none", "exact_action"]`,
  ...
  - [ ] Do **not** add `site_id`. The manifest is a declaration, not a site-owned resource
  ...
  - [ ] Define `CapabilityError(Exception)` with `code: str = "capability_error"` in this same
        module — the general base Decision 5 requires.
  - [ ] **Acceptance boundary:** a test asserts the frozen dataclass has exactly the declared
...
- [ ] **Task 2: Completeness validation — what "incomplete" means, as code** (AC: #1)
  - [ ] `validate_manifest(manifest) -> None`, raising `IncompleteManifestError` (a
  ...
  - [ ] Reject, each as its own case: any empty string field; `errors` empty; `evaluation_fixtures`
  ...
  - [ ] **Do not read the filesystem here.** `evaluation_fixtures` are validated as *shape* in the
  ...
  - [ ] Validation runs at **registration** — i.e. when `INSTALLED_MODULES` is composed and again
        in `compose_granted_capabilities` before anything is granted. An invalid manifest must
        never reach the renderer.
  - [ ] **Acceptance boundary:** a table-driven test over at least the ten rejection cases above,
...
- [ ] **Task 3: `CapabilityModuleV1` and the static installed set** (AC: #1, #2)
  - [ ] `backend/application/capabilities/module.py` — frozen dataclass `CapabilityModuleV1`:
  ...
  - [ ] `backend/application/capabilities/installed.py` — `INSTALLED_MODULES: tuple[CapabilityModuleV1, ...]`,
  ...
  - [ ] Module docstring states the AD-15 rule in one line so the next person does not "improve"
        it into a plugin loader.
  - [ ] Both new modules land inside `application/capabilities/`, which
  ...
  - [ ] **Acceptance boundary:** an architecture guard asserts no file under
...
- [ ] **Task 4: Generalize the registry — grant by declared requirement, not by name** (AC: #2)
  - [ ] Rewrite `compose_granted_capabilities` per **Decision 7**: keep the general clauses
  ...
  - [ ] Signature becomes `compose_granted_capabilities(context, modules=INSTALLED_MODULES) -> tuple[CapabilityModuleV1, ...]`.
  ...
  - [ ] Return granted **modules**, not manifests — the renderer needs the handler. Keep
        `resolve_granted_capability` working against the new element type.
  - [ ] `api/deps.py:88-101`'s `CapabilityComposer` alias and `get_capability_registry()` follow
        the new types. No route consumes it yet; that stays true (Story 2.7 owns the request path).
  - [ ] Keep `CapabilityGrantContextV1`'s docstring honest about which fields are still constants
        (`deferred-work.md:9`). Its wording is already correct — update it only where the shape
        actually changed.
  - [ ] **Acceptance boundary:** a matrix test over {granted, wrong role, missing feature policy,
...
- [ ] **Task 5: Generalize the renderer — delete `_RENDERERS`** (AC: #2, #3)
  - [ ] Rewrite `backend/agent/capability_tools.py` per **Decision 4**: one
  ...
  - [ ] `_RENDERERS` and the module-level `_RETRYABLE_CODES` are **deleted**. `UnknownCapabilityError`
  ...
  - [ ] Preserve exactly: `ctx.deps is None` → `RuntimeError("trusted agent dependencies are unavailable")`;
  ...
  - [ ] `RunContext`, `ModelRetry`, `Tool` and every other framework type stay inside this package.
  ...
  - [ ] **Acceptance boundary:** `test_a_granted_capability_with_no_renderer_fails_loudly`
...
- [ ] **Task 6: One error base, one failure-reason contract** (AC: #3)
  - [ ] `SchedulingInspectError` subclasses `CapabilityError`. No code, message, or `ERROR_CODES`
        value changes — verify by leaving every existing assertion in `test_scheduling_inspect.py`
        untouched and green.
  - [ ] `runtime.py:205` becomes `except CapabilityError`. **This is the only edit to `run_turn`'s
        control flow this story makes**, and it *removes* a per-capability branch rather than
        adding one — which is what AC3 requires.
  - [ ] Per **Decision 5**, address `AgentFailureReasonV1`: widen `AgentRunOutcomeV1.failure_reason`
  ...
  - [ ] **Acceptance boundary:** a test drives each installed module to raise its own declared
...
- [ ] **Task 7: Migrate the scheduling module onto the general contract** (AC: #4)
  - [ ] Delete `InspectCapabilityManifest`; `scheduling_inspect.py` imports `CapabilityManifestV1`.
  ...
  - [ ] Add the module's `CapabilityModuleV1` declaration beside `scheduling_inspect_manifest()`,
  ...
  - [ ] Update `PydanticAIAgentRuntime.__init__`'s `capabilities:` parameter type from
  ...
  - [ ] **Behaviour must not change.** Every assertion in `test_scheduling_inspect.py` about
  ...
  - [ ] **Acceptance boundary:** `test_scheduling_inspect.py` passes with no assertion weakened;
        `git diff` on the assertion bodies is limited to type/constructor renames.

- [ ] **Task 8: Promote `shiftmind_demonstration` into a governed module** (AC: #1, #3)
  - [ ] New `backend/application/capabilities/demonstration.py`: `DemonstrationRequestV1`
  ...
  - [ ] Manifest values, chosen deliberately: `risk_class="consequential"` (it is the only module
  ...
  - [ ] `idempotency_semantics` must be a real statement, not filler: the handler is pure over its
  ...
  - [ ] The handler takes `AgentDepsV1` like the scheduling handler (AC3: "uses trusted
  ...
  - [ ] **Approval behaviour is preserved exactly**: `repeat > 1` and not approved →
  ...
  - [ ] Delete the inline tool and `DemonstrationRequestV1` from `runtime.py:52-60, 125-140`.
        Update `registered_capability_names`' docstring — its "NOT COVERED" clause is now false.
  - [ ] **Acceptance boundary:** with the module granted, a scripted double reproduces both
...
- [ ] **Task 9: The conformance suite — parametrized over every installed module** (AC: #1, #2, #3)
  - [ ] New `backend/tests/test_capability_conformance.py`, parametrized over `INSTALLED_MODULES`
  ...
  - [ ] Per module, assert: manifest passes `validate_manifest`; `input_schema_ref`/`output_schema_ref`
        ...
        finding 7).
  - [ ] Assert the grant-by-absence invariant per module: ungranted → the tool name is absent from
        `agent._function_toolset.tools`, and a model-supplied call to that name cannot execute.
  - [ ] **Include a self-redness proof.** Story 2.5's review found a guard shipped without one
        ...
        rejects it.
  - [ ] **Acceptance boundary:** the suite is green over both installed modules, and demonstrably
        red for a synthetic module missing any one required declaration.

- [ ] **Task 10: The removal proof** (AC: #4)
  - [ ] Per **Decision 1**, a test composes `compose_granted_capabilities(context, modules=<INSTALLED_MODULES without demonstration>)`
  ...
  - [ ] Assert **no core file names the demonstration module**: `agent/runtime.py`,
  ...
  - [ ] Assert the **historical-record clause** directly: `evidence/story-2.2/evaluation-harness-demonstration.json`
  ...
  - [ ] **Do not** delete files, and **do not** regenerate the Story 2.2 evidence file
        (`2-5-…md:696-706`).
  - [ ] **Acceptance boundary:** all three assertions green, and the removed-world composition is
        exercised by a real `PydanticAIAgentRuntime` rather than asserted about a tuple.

- [ ] **Task 11: Eval harness, golden cases, and the NFR28 exemption** (AC: #4)
  - [ ] The two demonstration golden cases are **unchanged on disk** (their sha256 is pinned by
  ...
  - [ ] **This also fixes a live pre-existing hole, so do not treat it as refactoring.** The five
  ...
  - [ ] `report.py:32-40`'s `DEMONSTRATION_BINDINGS["tool"]` still says "from the Story 2.1
  ...
  - [ ] **Confront the NFR28 exemption rather than inheriting it.** `test_evaluation_harness.py:312-317`
  ...
  - [ ] `test_evaluation_harness.py:301`'s `"demonstration" in {…}`, the `>= 2` assertion, and
  ...
  - [ ] **Acceptance boundary:** `uv run --frozen python -m evals.report` (or its existing entry
...
- [ ] **Task 12: Fences, ledger, baselines, Gate A** (AC: #4)
  - [ ] **Zero-line diff required** in: `backend/agent/translate.py`, `backend/services/**`,
  ...
  - [ ] `deferred-work.md:10` is the entry that names this story by owner. **Close it** — its
  ...
  - [ ] Do **not** close: the `translate.py` silent-drop item (`:100`, owner is the first story
  ...
  - [ ] Add a ledger entry for the eval-report pass-signal gap (`deferred-work.md:108`) if this
        story's report changes touch it, and for the `AgentFailureReasonV1` widening if Task 6 defers.
  - [ ] Re-derive regression baselines rather than trusting the recorded ones — that is this
  ...
  - [ ] **Re-run Gate A per AR28** (`docs/GATE-A-RUNBOOK.md` §3) and confirm `gate_a_passed: true`
  ...
  - [ ] **Acceptance boundary:** every fence verified empty by `git diff --stat`, the ledger
