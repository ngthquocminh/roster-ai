...
- **`record_id` is intentionally not a rendered column for `work-areas-and-tasks` or `workers`** (code review, 2026-08-06). AC #1 names `record_id` among the stable identifiers that "must render exactly," but for these two groups it is used only as the React list `key`; the rendered, parity-checked business keys are `task_id` and `contact_id` respectively (both stable, both exact-rendered, both checked by `ScenarioDataParity.test.tsx`). This mirrors `columns.ts`'s existing design for these two groups (Story 1.7/1.8) and is accepted as-is rather than adding a redundant visible `record_id` column.
...
Reviewed diff: scoped subset of `git diff e2b2038..366bd4e` — backend scripts/tests, frontend new tests, and story/evidence docs (11 files, 1071 lines). `data/contract/*.projection-v1.json` (32,768 lines, generated fixture snapshots) and the 6,073-line legacy-component deletion (pure removal) were excluded from line-by-line review per user direction; the deletion was instead spot-checked for dangling references during triage. Layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor.

- [x] [Review][Decision] AC #1's "stable identifiers... must render exactly" is never exercised for `record_id` in the `work-areas-and-tasks` and `workers` groups — `record_id` is used only as the React `key`, never rendered as a cell or listed in `columns.ts` for these two groups, so `ScenarioDataParity.test.tsx` cannot and does not check it there, even though the story text explicitly names `record_id` as a stable identifier that must render exactly — `frontend/src/features/scenario-data/columns.ts:18-36`, `frontend/src/features/scenario-data/ScenarioDataParity.test.tsx` — **resolved 2026-08-06: accepted as documented deviation.** `task_id`/`contact_id` are the natural, already-parity-checked business keys for these two groups; `record_id` stays an internal React key only. See Dev Notes.
- [x] [Review][Patch] Orphaned legacy API-client wrappers `frontend/src/api/scenarios.ts`, `constraints.ts`, `runs.ts` still contain live `client.POST(...)` mutation calls, have zero importers left in the live tree (their only callers were the hooks this story already deleted), and are covered by neither `legacyReachability.test.ts`'s `LEGACY_MODULES` list nor `scenarioDataBoundaries.test.ts`'s GET-only scan — contradicts the Task 5 completion note ("deleted the unreachable legacy UI and mutation hooks") [frontend/src/api/scenarios.ts:41, frontend/src/api/constraints.ts:13, frontend/src/api/runs.ts:28] — **fixed 2026-08-06:** added the three files to `legacyReachability.test.ts`'s `LEGACY_MODULES`, confirmed unreachable, deleted them and their `.test.ts` siblings; evidence file's frontend test count updated to 47/200.
- [x] [Review][Patch] `scenarioDataBoundaries.test.ts`'s mutation-affordance check only matches mutating verbs inside an `aria-label` attribute, not a button's actual accessible name (which falls back to text content) — a future `<button>Delete row</button>` with no `aria-label` would pass undetected; the stricter per-panel `panelTestContract.tsx` check (aria-label ?? textContent) doesn't cover `FilterBar.tsx`/`ColumnChooser.tsx`/`ScenarioDataView.tsx`, which this test is the only guard for [frontend/src/test/scenarioDataBoundaries.test.ts:35] — **fixed 2026-08-06:** the check now also scans rendered `<button>...</button>` text content for the same mutating-verb word list.
- [x] [Review][Defer] `evidence/story-1.9/gate-a-viewer-parity-and-mutation-denial.json`'s `code.git_commit` records the pre-commit parent hash (`e2b2038`), not the actual commit (`366bd4e`) containing the story's own changes — deferred, pre-existing: `evidence/story-1.4/nfr35-scenario-data-load.json` and `evidence/story-1.5/nfr35-evidence-target-resolution.json` show the identical pattern, so this is an established (if imperfect) repo-wide evidence convention, not a defect introduced by this story [evidence/story-1.9/gate-a-viewer-parity-and-mutation-denial.json:55]
- [x] [Review][Defer] `legacyReachability.test.ts`'s regex-based import walker (`IMPORT_PATTERN`) cannot trace dynamic `import(...)` expressions, so a future legacy/mutation module reachable only via dynamic import would be silently reported unreachable — deferred, no dynamic import exists anywhere in the frontend today so nothing is exploitable now, but this is the sole gate for an irreversible bulk deletion and worth hardening before a future story relies on it again [frontend/src/test/legacyReachability.test.ts:29]
