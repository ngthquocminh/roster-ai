...
Code review 2026-08-24. Verified against the working tree at `74e6e29`; all 15 new
backend tests and the 53 touched frontend tests pass.

- [x] [Review][Patch] **AC2's structural proof did not observe the manual solver path's actual `agent.runtime` import** — `MANUAL_SOLVER_PATHS` scanned 11 files and only their *direct* imports. `api/routers/schedule_runs.py:13` imports `api.deps`, and `api/deps.py:39` is `from agent.runtime import create_agent_runtime`, so the manual solver path did import `agent.*` one hop out while the test stayed green; the detector self-test only proved the matcher works on a synthetic direct import, never that the scan set was closed. **Resolved 2026-08-24 — option 2 (transitive scan + exact allow-list).** `_agent_bridges()` now walks transitively from the seeds and stops at the boundary; `ACCEPTED_AGENT_BRIDGES` is a frozen, exact set (`api/deps.py`, `application/use_cases/execute_turn.py`) asserted by equality, mirroring `LEGACY_ENGINE_IMPORTERS`, so a third bridge cannot silently join. Added `test_transitive_walk_observes_a_bridge_the_seed_scan_cannot_see` so the walker cannot pass vacuously, and renamed the seed test to `..._has_no_direct_agent_runtime_import` so its name states what it proves. Both bridges are import-time only — AC2's behavioral raising-factory proof shows neither is reached at runtime. Option 1 (splitting `api/deps.py`) was costed at ~2–4 h across 2 routers, 1 new module and 5 test modules, and would have required editing `api/routers/conversations.py`, which this story lists as untouchable. 7/7 tests pass. [backend/tests/architecture/test_model_outage_boundaries.py:56]

- [x] [Review][Defer] **The availability query has no supporting index and scans the site's whole append-only event table** [backend/adapters/postgres/conversation.py:63] — deferred, accepted for the MVP. `persisted_event` carries only `ix_persisted_event_site_id` (`schema.py:342`), while the new read filters `site_id` + `agent_run_id IS NOT NULL` + `event_type = 'terminal_outcome'` and orders by `occurred_at DESC, id DESC`, so Postgres scans and sorts every event in the site partition. `frontend/src/main.tsx:13` uses a default `QueryClient` (`staleTime: 0`, `refetchOnWindowFocus: true`), firing it on every ChatView mount and window focus. Single site and small event volume for this portfolio MVP, and the fix is a `(site_id, event_type, occurred_at DESC)` migration that Story 3.9 deliberately scoped out. Owner/revisit trigger recorded in `deferred-work.md`.

- [x] [Review][Patch] **`test_manual_run_result_and_evidence_survive_a_raising_trace_exporter` could not go red** — FIXED. — the `TracerProvider` is local: never set as the global provider, never injected into anything the deterministic flow touches. Grep confirms zero `opentelemetry` / tracer usage in `api/routers/schedule_runs.py`, `application/use_cases/enqueue_compute.py`, `worker/lease_worker.py`, and `engine/`. Deleting the whole provider block leaves the test passing identically, so Task 6's third bullet ("a raising exporter **installed**") is not satisfied and Decision E's own rule — do not ship a test that asserts a fake's behavior — is broken. The deterministic path having *no* instrumentation is in fact the strongest possible independence; assert that structurally (no `opentelemetry` import on the manual solver path) instead of miming an install. [backend/tests/architecture/test_model_outage_boundaries.py:425]

- [x] [Review][Patch] **The new adapter read `latest_terminal_outcome_for_site` had zero real-database coverage** — FIXED. — only the `_AvailabilityRepository` fake in `tests/test_agent_availability.py:64` exercises it. Every other `PostgresConversationRepository` read (`timeline`, `events_after`, `list_for_scenario`, `claim_queued_run`) is covered against a real database in `tests/test_conversations_postgres.py`. Trap 5 named this exact risk: a wrong column, filter, or ordering ships green because no test ever runs the SQL. Add a case to `test_conversations_postgres.py` covering a `provider_error` terminal event, a newer non-provider event shadowing it, and cross-site isolation under RLS. [backend/adapters/postgres/conversation.py:58]

- [x] [Review][Patch] **The AC3 telemetry test never asserted a span was actually exported** — FIXED. — `InMemorySpanExporter` drives the control run but `get_finished_spans()` is never checked. Spans are emitted today (verified: 2 — `invoke_agent agent`, `chat function:scripted:`), so the test is live. But if instrumentation ever stops emitting — a pydantic-ai upgrade, an `InstrumentationSettings` change — all three runs become trivially identical and this NFR10 proof silently goes vacuous. Assert the control exporter received at least one span. [backend/tests/test_conversations_api.py:585]

- [x] [Review][Patch] **`test_solver_service_failure_keeps_its_own_problem_surface` ended in a tautology** — FIXED. — `assert response.json()["code"] != "agent_unavailable"` cannot fail: the preceding line already asserts the code equals `"scenario_unavailable"`. It reads as the `EXPERIENCE.md:270` non-conflation proof but asserts nothing. Either drop it or make it real by asserting the response body carries none of the model-outage surface's copy. [backend/tests/architecture/test_model_outage_boundaries.py:503]

**Patch resolutions (2026-08-24).** `test_manual_solver_path_imports_no_telemetry_at_all` replaces the orphan-provider theatre: it walks the manual solver path transitively and asserts no first-party module reaches `opentelemetry` or `logfire` at all — independence proven by absence, and red-able the moment an import is added. The behavioral half is now `test_manual_run_result_and_evidence_survive_a_failing_span_exporter`, which ends spans either side of the flow (`SimpleSpanProcessor` exports on span *end*, so the previous single wrapping span raised only after the work) and asserts `exporter.calls == 2`. The AC3 conversation test now asserts its control `InMemorySpanExporter` received a span. The tautological `!=` assert became a real check that no model-outage copy appears in the solver-service problem body. `test_latest_terminal_outcome_for_site_reads_the_newest_typed_agent_failure` covers the new adapter read against a live migrated PostgreSQL — empty case, typed reason through JSONB, newest-wins, and site isolation — and was red-proofed by breaking the `event_type` filter. Full backend suite: **1216 passed, 2 skipped** (was 1212).

- [x] [Review][Defer] **`aria-describedby` on a `disabled` control does not reach assistive technology** [frontend/src/features/chat/Composer.tsx:61] — deferred, spec-level. A disabled textarea/button is unfocusable, so most AT never announces its accessible description, and the polite live region is mounted at the same instant as its content, which AT commonly does not announce either. `Composer.submit` already guards on `disabledReason` (`Composer.tsx:39`), so `aria-disabled="true"` would keep both controls focusable and announceable without permitting submission. The implementation follows Decision C verbatim — the flaw is in the decision, not the code — and `EXPERIENCE.md:185-195` descopes manual AT verification, making the passing axe check the accepted proof bar. Recorded rather than changed unilaterally.
