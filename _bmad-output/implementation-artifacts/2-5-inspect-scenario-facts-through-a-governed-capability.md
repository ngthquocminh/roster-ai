...
Code review 2026-08-11 (three adversarial layers, all reporting; findings verified
against source, several reproduced by executing the code against the pinned
pydantic-ai 2.27.0).

- [x] [Review][Decision] **RESOLVED 2026-08-11 → option (a) + `ModelRetry`.** The port exposes its legal sort/filter keys per group; the application allow-lists against that rather than re-deriving the tables (Trap 4 preserved — the adapter remains the single source-data interpretation). An invalid query is surfaced to the model as a retryable error so it self-corrects, rather than as a hard refusal (which would pre-empt Story 2.9's scope). **Implementation constraint:** `ModelRetry` is a `pydantic_ai` type and `application/**` may not import the framework — so the handler raises a typed application error (`invalid_query`, already in the manifest vocabulary) and the `backend/agent/` wrapper maps it to `ModelRetry`. **Verified safe:** evidence `contract_digests` bind fixture data files (`data/contract/*.projection-v1.json`), not the port module, so extending the Protocol causes no evidence drift and no Gate A re-measurement. Original finding follows.
- [x] [Review][Context] **Where does model-supplied query validation live?** — `SchedulingInspectRequestV1.sort` is a free-form `str` and `.filters` is `tuple[tuple[str, str | int], ...]`; both reach `sorts[query.sort]` and `filters[name](item, value)` in `adapters/postgres/scenario_projection.py:432,440`, which are unguarded dict subscripts. Reproduced: `sort="not_a_column"` → `KeyError`; `sort="scope"` (a valid *lock* sort) against demand → `KeyError`; `("start_minute_gte","abc")` → `TypeError`. The HTTP path is protected by per-group `sort: Literal[...]` enums (`api/routers/scenario_projection.py:211,257,307,355,398,443`); the agent path reproduces none of it, so the governed capability is strictly weaker than the browser against the same adapter. The manifest declares an `invalid_query` error (`application/capabilities/scheduling_inspect.py:93`) that no code path raises — repo-wide it has exactly one occurrence. **The tension needing your call:** the sort/filter tables are owned by the adapter, and Trap 4 forbids the application re-deriving them ("two interpretations of the same fixture"). Options: (a) the port exposes its legal sort/filter keys per group so the application can allow-list without reinterpreting; (b) the adapter raises a typed `invalid_query` the handler maps; (c) constrain `sort`/`filters` to per-group `Literal`s on the request dataclass and let pydantic reject at the tool boundary. Also decide whether an invalid query is a retryable `ModelRetry` (agent self-corrects) or a hard typed refusal — the latter overlaps Story 2.9's refusal scope.
- [x] [Review][Decision] **RESOLVED 2026-08-11 → option (c), both.** Add a symmetric `site_mismatch` check beside the existing `version_mismatch` check, with the new error added to the manifest's `errors` vocabulary; **and** record connection/RLS as the authoritative site control with the non-coverage stated as **data** (following the `ALLOWED_LEAKS` pattern), not as a docstring claim. Rationale: the check is cheap, symmetric with the version check already shipped, and Story 2.7 puts this capability on a request path — at which point the gap becomes user-facing. **Mandatory companion (independent of this choice):** the test fixture must give each identity field a distinct UUID; while `_deps()` and `ProjectionStub` share one `identity`, no site or version assertion can prove anything. Tracked as its own patch. Original finding follows.
- [x] [Review][Context] **Is site scope enforced by the capability or by the connection?** — AC4 requires "every tool result remains site- and version-scoped". The version half is enforced (`application/capabilities/scheduling_inspect.py:117-118` raises `version_mismatch`); the site half has no counterpart. `deps.site_id` is read nowhere in `application/capabilities/` — grep-confirmed; the result echoes `str(value.site_id)` from whatever the projection returned, and the manifest claims `scope="current_site/current_scenario_version"` (`:85`). The test that appears to cover it (`tests/test_scheduling_inspect.py:99`) passes by construction because `_deps()` assigns one `uuid4()` to every identity field including the stub's page — it would still pass if the handler ignored site entirely. **The tension needing your call:** Dev Notes name `site_context()` (`api/deps.py:140-177`) as *the* single place trusted site context is established, so RLS on the connection may be the intended primary control and a handler check would be defence-in-depth. Options: (a) add a symmetric `site_mismatch` check plus the error in the manifest vocabulary; (b) record the connection as the authoritative control and state the non-coverage as data; (c) both. Note `AgentDepsV1.connection` is typed `Any` and supplied by a caller that does not exist yet, so today nothing verifies the connection is site-scoped either.
- [x] [Review][Patch] *(from Decision 1)* Extend `ScenarioProjectionReader` to expose legal sort/filter keys per group; allow-list `sort`/`filters` in the handler against that, raise the declared `invalid_query`, and map it to `ModelRetry` in the `backend/agent/` wrapper [backend/application/ports/scenario_projection.py:102-129, backend/application/capabilities/scheduling_inspect.py:101-107, backend/agent/runtime.py:146-160]
- [x] [Review][Patch] *(from Decision 2)* Add a symmetric `site_mismatch` check plus the error in the manifest vocabulary, and record connection/RLS as the authoritative site control with non-coverage stated as data [backend/application/capabilities/scheduling_inspect.py:93, 115-118]
- [x] [Review][Patch] Handler and tool exceptions escape `run_turn` as raw builtins, breaking the adapter's owned-error contract [backend/agent/runtime.py:186-217]
- [x] [Review][Patch] `registered_capability_names` mirrors the constructor argument instead of the Agent's real tool set, and capability rendering is a hardcoded `if name == "scheduling_inspect"` with no `else` and no renderer module [backend/agent/runtime.py:101-104, 146-160]
- [x] [Review][Patch] `limit <= 0` and negative `cursor` are passed through unclamped, silently returning a wrong window and a non-terminating paging loop [backend/application/capabilities/scheduling_inspect.py:101-107]
- [x] [Review][Patch] Task 5's "all seven fact kinds" boundary is unmet — only `get_demand` is stubbed; the `overview` and `locks` groups and both declared error paths have zero coverage, and the eval harness stub omits `get_overview` entirely [backend/tests/test_scheduling_inspect.py:37-49, backend/tests/test_evaluation_harness.py:110-125]
- [x] [Review][Patch] The "handler imports no adapter" guard walks only `ast.Import`, never `ast.ImportFrom`, so it can never go red against a module that uses `from … import …` exclusively [backend/tests/test_scheduling_inspect.py:103-105]
- [x] [Review][Patch] Scope assertions are vacuous — one `uuid4()` is reused for every deps field and every stub page field, so site/version assertions cannot detect a mix-up or a re-stamp [backend/tests/test_scheduling_inspect.py:52-61]
- [x] [Review][Patch] Guard quality below Task 8's own bar — no self-redness test for the new prohibited-capability guard, a hand-maintained name list Task 8 explicitly forbade, `application/capabilities/**` enumerated file-by-file rather than swept, and no scope-as-data record of non-coverage [backend/tests/test_scheduling_inspect.py:125-134, backend/tests/architecture/test_conversation_boundaries.py:21-25, 61]
- [x] [Review][Patch] Golden cases declare NFR27 fixture bindings they never exercise (the harness injects an empty stub), no executable assertion enforces the ≥4 floor, none filters to Wednesday, and the `locks` group is unrouted [backend/evals/golden/scheduling_inspect/*.json, backend/tests/test_evaluation_harness.py:110-137, 284]
- [x] [Review][Patch] The two new settings use bare `int()`/`float()` where every neighbour uses `_optional_int`/`_optional_float`, so a blank or malformed env var raises at request time — the manifest is rebuilt on every tool call [backend/settings.py:186-191]
- [x] [Review][Patch] `timeout_seconds` and `remaining_budget` are declared and never consumed — nothing bounds the projection read to the advertised 5s and nothing consults the budget before executing [backend/application/capabilities/scheduling_inspect.py:68, 89; backend/application/capabilities/deps.py:29]
- [x] [Review][Patch] Minor cluster — manifest rebuilt per tool call re-reading `os.environ`; `read_scenario_facts` returns `object | None` via `getattr` dispatch and raises `KeyError` for the real `"tasks"` group; `CapabilityGrantContextV1.conversation_id` is declared and never read; `get_capability_registry()` is unannotated and returns the function rather than a composed grant [backend/application/use_cases/read_scenario_facts.py:24-33, backend/application/capabilities/registry.py:25, backend/api/deps.py:84-86]
- [x] [Review][Defer] `frontend/playwright.config.ts` breaks the mandated zero-line frontend fence with a hardcoded `workers: 4` and no CI branch [frontend/playwright.config.ts:6-11] — deferred, pre-existing
