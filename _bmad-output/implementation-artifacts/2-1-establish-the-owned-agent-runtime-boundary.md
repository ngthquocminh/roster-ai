...
- [x] [Review][Defer] `create_agent_runtime()` never wires `config.model`/`config.api_key` into a real PydanticAI model — the factory reads `settings.agent_runtime_model`/`agent_runtime_api_key` (`backend/agent/runtime.py:298-307`) but `AgentRuntimeConfig.model`/`.api_key` are stored and never read again; the only thing that ever becomes `self._model` is the constructor's `model=` kwarg, which every test in this diff supplies as a `FunctionModel` double. Calling `create_agent_runtime()` with no injected `model=` produces a runtime whose `run_sync(model=None)` would raise a raw framework error, not the owned `AgentRuntimeError` — and `create_agent_runtime()` itself is untested and unreferenced outside `backend/agent/__init__.py`'s export. Nothing in this diff wires it to a live endpoint, so today's blast radius is zero, but it's the exported production factory and a real trap for whichever story calls it first. [backend/agent/runtime.py:68-69, 90-91, 288-308] — deferred: out of scope for this enabler story; no endpoint calls `create_agent_runtime()` yet, live model construction belongs to the story that first wires `AgentRuntime` into a real request path
- [x] [Review][Defer] `timed_out` and `budget_exhausted` outcomes never populate `turn` — the `except RunCancelled`/`except UsageLimitExceeded` branches return `AgentRunOutcomeV1(status=..., summary=...)` with `turn` defaulting to an empty `AgentTurnV1()`, discarding any tool-call progress made before the cancellation/exhaustion. The adapter never wraps `run_sync` in PydanticAI's `capture_run_messages()`, so there is currently no way to recover partial history on that path even if desired. [backend/agent/runtime.py:151-170] — deferred: enhancement, not required by any AC in this story; revisit when a story needs to debug/display a timed-out run
- [x] [Review][Patch] Architecture-boundary test docstring overclaims its own coverage — it states domain/application "must not import FastAPI, PydanticAI, SQLAlchemy, Cognito, S3, Logfire, or concrete model providers" (citing AD-1), but `FORBIDDEN_ROOT_MODULES` only actually checks `("pydantic_ai", "pydantic_graph", "logfire")`. A stray `import sqlalchemy` or `from fastapi import ...` inside `domain/**`/`application/**` would pass this guard silently despite the docstring's claim. [backend/tests/architecture/test_agent_runtime_boundaries.py:1-9, 48] — fixed: docstring narrowed to state it only enforces the agent-runtime seam's own dependencies, not repo-wide AD-1
- [x] [Review][Patch] `_to_deferred_results` uses truthiness (`decision.denial_reason or "denied by the application"`), so an explicit empty-string `denial_reason=""` is silently replaced by the generic fallback text instead of being preserved as the (unusual but valid) recorded reason. [backend/agent/runtime.py:266-271] — fixed: switched to an explicit `is not None` check
- [x] [Review][Defer] `_merge_budget` overloads `None` as both "field not specified" and "explicitly unlimited" — a caller cannot request "no limit" and have it win over a configured default, unlike `settings.py`'s `_optional_int`/`_optional_float` tri-state parsing which was written to avoid exactly this ambiguity. [backend/agent/runtime.py:213-238] — deferred, pre-existing category of gap with no current caller depending on the distinction
- [x] [Review][Defer] `_to_deferred_results` has no duplicate-`tool_call_id` detection — two approval decisions sharing an id in one request silently collapse to the last one with no error or log. [backend/agent/runtime.py:257-272] — deferred, requires malformed/duplicate caller input not produced by any current path
- [x] [Review][Defer] The `except AgentRunError` catch-all assumes every real provider failure surfaces as a PydanticAI exception type; whether a raw transport exception (e.g. from the underlying `httpx`/`openai`/`google-genai` SDK) could cross the seam unwrapped during a live call is unverified — this diff runs `models.ALLOW_MODEL_REQUESTS = False` throughout with no live-provider test. [backend/agent/runtime.py:151-178] — deferred, needs live-provider coverage this story deliberately excludes
- [x] [Review][Defer] `to_framework_messages` silently drops an unrecognized `AgentMessageV1.role` (no `else`) and `_from_response`'s counterpart silently skips an unrecognized `AgentPartV1.kind` inside the assistant branch — both unreachable today since `to_owned_turn` is the only producer, but become reachable once persisted/deserialized turns exist. [backend/agent/translate.py:61-105] — deferred, real persistence/deserialization boundary belongs to Story 2.3
- [x] [Review][Defer] `test_the_guarded_layers_are_not_empty` checks the *combined* `domain`+`application` file count (`>5`), not each layer individually — `domain` could theoretically be emptied while `application` alone satisfies the count, silently disabling all domain-layer checks. [backend/tests/architecture/test_agent_runtime_boundaries.py:227-233] — deferred, meta-test hardening, not exploitable without an adversarial deletion that would break many other tests first
- [x] [Review][Defer] `find_framework_typed_contract_fields` inspects annotation/default AST nodes but never string-literal (quoted forward-reference) annotations, so a hypothetical `raw: "ModelResponse | None"` field would bypass the AC3 contract-field guard. No current contract field uses this style. [backend/tests/architecture/test_agent_runtime_boundaries.py:165-186] — deferred, no current instance, narrow edge case
- [x] [Review][Defer] `test_wall_time_exhaustion_is_timed_out_not_budget_exhausted` asserts a `timed_out` outcome using a real `time.sleep(0.3)` against `deadline_seconds=0.05` — wall-clock-timing-dependent and a plausible (unconfirmed) source of future CI flakiness under load, despite the ~6x margin. [backend/tests/test_agent_runtime_adapter.py:209] — deferred, passed in the recorded Gate A run, margin is reasonably wide
- [x] [Review][Defer] `AgentBudgetV1.total_tokens_limit` has no corresponding `agent_runtime_*` setting/env var, unlike `request_limit`, `tool_calls_limit`, and `deadline_seconds` — it can only be set programmatically per-request, never operationally configured. [backend/settings.py:67-74, 173-208] — deferred, minor config-surface asymmetry
...
| 2026-08-10 | Code review (`bmad-code-review`, three parallel layers: adversarial, edge-case, acceptance-auditor). No AC violations found; both explicitly-flagged anti-pattern traps confirmed avoided; dependency-lock claim independently re-verified against `backend/uv.lock`. 2 decision-needed findings both deferred (model wiring in `create_agent_runtime()`, partial-turn capture on timeout — both out of scope for this enabler story). 2 patch findings fixed and verified (485/486 backend suite green, 1 self-skip from dirty tree matching Story 1.11's documented pattern): `ToolDenied` truthiness bug on empty `denial_reason`, and the architecture-boundary test docstring narrowed to match its actual `FORBIDDEN_ROOT_MODULES` scope. 8 low-severity findings deferred to `deferred-work.md`. Status → done. |
