...
- [x] Task 1: Define the `EvidenceRefV1` contract type (AC: #1, #2)
...
- [x] Task 2: Port method + PostgreSQL adapter — exact-target resolution (AC: #1, #2, #3)
  - [x] Extend the existing `ScenarioProjectionReader` Protocol (`backend/application/ports/scenario_projection.py`) — do not create a second reader Protocol; Story 1.4's Unblocks line for this story says evidence-target resolution "reuses this same read path." Add one method per non-overview group, e.g.:
    ```python
    def resolve_task(
  ...
  - [x] Resolution algorithm per group (identical shape across all six — extract a shared private helper if that reads cleaner, e.g. `_resolve(items, requested_scenario_version_id, actual_scenario_version_id, record_id) -> tuple[outcome, item | None]`):
...
- [x] [Review][Patch] Bare `assert resolution.item is not None` used as control flow in all six `resolve_*` handlers — stripped entirely under `python -O`, and doesn't fail loudly if `ResolutionOutcomeV1` ever gains a fourth value [backend/api/routers/scenario_projection.py:373,411,449,487,525,563] — fixed: replaced with an explicit `if resolution.item is None: raise HTTPException(status_code=500)` inside the new shared `_resolve_or_problem` helper
- [x] [Review][Patch] ~20-line `not_found`/`version_mismatch`/`problem_response` block duplicated verbatim across all six `resolve_*` handlers — the adapter extracted a shared `_resolve_items` helper for the equivalent logic, but the router repeats itself six times [backend/api/routers/scenario_projection.py:356-372,394-410,...] — fixed: extracted `_resolve_or_problem(resolution, mapper)` and each handler now calls it with its `_xxx_out` mapper
- [x] [Review][Patch] `evidence/story-1.5/nfr35-evidence-target-resolution.json` is untracked in git, unlike the committed `evidence/story-1.4/nfr35-scenario-data-load.json` precedent — AC #3's "recorded as release evidence" isn't durably satisfied until this file is staged and committed [evidence/story-1.5/nfr35-evidence-target-resolution.json] — fixed: staged with `git add`
- [x] [Review][Defer] Unauthorized-case non-disclosure test coverage exists for only one of six evidence groups (`work-areas-and-tasks`), and only via the fixture-backed reader, not a real cross-site Postgres scenario [backend/tests/test_scenario_projection.py:779] — deferred, optional hardening explicitly permitted by this story's own Task 5 wording, not a violation
- [x] [Review][Defer] `_resolve_items` takes the first matching candidate without detecting duplicate `record_id`s among normalized items [backend/adapters/postgres/scenario_projection.py:331] — deferred, pre-existing assumption inherited from the same `_normalize_*` functions Story 1.4's paginated endpoints already rely on, not newly introduced here
...
- Follow the story task order with red-green-refactor validation per task.
- Define the neutral evidence contracts before extending the existing reader port and PostgreSQL adapter.
- Reuse the normalized projection records and router mappers for exact-target endpoints.
- Regenerate OpenAPI types, capture NFR35 evidence, then run the complete backend/frontend quality gates.

### Debug Log References

- Task 1 RED: `uv run --frozen pytest tests/test_evidence_ref.py` failed during collection because `application.contracts.evidence_ref` did not exist.
- Task 1 GREEN/regression: 2 focused tests passed; full backend suite passed (302 passed, 6 deselected).
- Task 2 RED: 4 focused failures confirmed the resolver methods were absent from the port and adapter.
- Task 2 GREEN/regression: 6 focused tests passed; full backend suite passed (306 passed, 6 deselected).
- Task 3 RED: exact-target API checks failed with 404s and only 7 projection paths before route implementation.
- Task 3 GREEN/regression: 26 focused projection tests passed; full backend suite passed (308 passed, 6 deselected); frontend typecheck and production build passed after code generation.
- Task 4 RED: the release-evidence validation failed because the Story 1.5 evidence artifact did not yet exist.
- Task 4 GREEN/measurement: PostgreSQL-backed shallow and deepest-target measurements passed all six requests with a 44.344 ms maximum; evidence validation and the full backend suite passed (310 passed, 6 deselected).
- Task 5 focused validation: 33 evidence/projection tests passed after adding the cursor-1500 paging cross-check.
- Task 5 final validation: 311 backend tests passed (6 live-provider tests deselected); Alembic found zero drift; codegen, typecheck, lint, and production build passed. Lint retained four warnings in untouched legacy/frontend primitive files.

### Completion Notes List

- Task 1: Added the frozen `EvidenceRefV1` normative locator and six group-specific resolution outcome contracts in a neutral, transport-free module.
- Task 1 judgment call: retained nullable `producing_run_version` and `baseline_schedule_version` fields while leaving population to the owning future epics.
- Task 2: Extended the existing projection reader with six version-bound resolvers, reusing current-version lookup and the established normalization paths without historical payload access.
- Task 2: Verified deep demand resolution, version mismatch, not-found, site-scoped absence, all populated groups, and always-empty assignment/lock groups.
- Task 3: Added six required GET-only synchronous endpoints with required version IDs, existing item mappers, distinct authorized RFC 7807 failures, and generic non-disclosing unauthorized 404s.
- Task 3: Regenerated the OpenAPI document and TypeScript client declarations; no frontend feature code was added.
- Task 4: Recorded the NFR35 protocol and three warm runs each for demand indexes 0 and 1546; every result was comfortably below the 2,000 ms acceptance threshold.
- Task 5: Completed all success, failure-separation, non-disclosure, domain-purity, mutation-denial, sync-handler, empty-group, migration, generated-contract, and regression checks.
- Task 5 refactor: made normalization lazy so a version mismatch returns before payload normalization or record search, with a dedicated regression test.

### File List

- _bmad-output/implementation-artifacts/1-5-resolve-exact-evidence-targets.md (modified)
- _bmad-output/implementation-artifacts/sprint-status.yaml (modified)
- backend/application/contracts/evidence_ref.py (new)
- backend/application/ports/scenario_projection.py (modified)
- backend/adapters/postgres/scenario_projection.py (modified)
- backend/api/routers/scenario_projection.py (modified)
- backend/tests/test_evidence_ref.py (new)
- backend/tests/test_scenario_projection.py (modified)
- backend/tests/test_postgres_integration.py (modified)
- evidence/story-1.5/nfr35-evidence-target-resolution.json (new)
- frontend/openapi.json (generated)
- frontend/src/api/schema.d.ts (generated)

## Change Log

- 2026-08-05: Implemented Story 1.5 exact evidence contracts, version-bound projection resolvers, six read-only API routes, generated client contracts, comprehensive tests, and passing NFR35 release evidence.
